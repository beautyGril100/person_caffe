...
int AnchorGenerator::Init(int stride, const AnchorCfg& cfg, bool dense_anchor) {
    ...
    printf("func:%s,line:%d, anchor_num:%d, anchor_stride:%d\n", __FUNCTION__, __LINE__, anchor_num, anchor_stride);
    for (int i = 0; i < anchor_num; ++i) {
...
int AnchorGenerator::FilterAnchor(caffe::Blob<float>* cls, caffe::Blob<float>* reg, caffe::Blob<float>* pts, std::vector<Anchor>& result) {
    printf("func:%s, line:%d, anchor_num:%d\n", __FUNCTION__, __LINE__, anchor_num);
    //assert(cls->channels() == anchor_num*2);
    //assert(reg->channels() == anchor_num*4);
    int pts_length = 0;

	assert(pts->channels() % anchor_num == 0);
	pts_length = pts->channels()/anchor_num/2;
...
//            	std::cout<< j << i << id<<cls.channel(anchor_num + a)[id]<<",";
            	if (cls->cpu_data()[(anchor_num + a) * h * w + id] >= cls_threshold) {
                    printf("cls %f\n", cls->cpu_data()[(anchor_num + a) * h * w + id]);
                    ...
                    CRect2f delta(reg->cpu_data()[(a*4+0) * reg->height() * reg->width() + id],
                            reg->cpu_data()[(a*4+1) * reg->height() * reg->width() + id],
                            reg->cpu_data()[(a*4+2) * reg->height() * reg->width() + id],
                            reg->cpu_data()[(a*4+3) * reg->height() * reg->width() + id]);

                    Anchor res;
                    res.anchor = cv::Rect_< float >(box[0], box[1], box[2], box[3]);
                    bbox_pred(box, delta, res.finalbox);
                    printf("bbox pred\n");
                    res.score = cls->cpu_data()[(anchor_num + a) * h * w + id];
                    ...
                    if (1) {
                        ...
                        for (int p = 0; p < pts_length; ++p) {
                            pts_delta[p].x = pts->cpu_data()[(a*pts_length*2+p*2) * pts->height() * pts->width() + id];
                            pts_delta[p].y = pts->cpu_data()[(a*pts_length*2+p*2+1) * pts->height() * pts->width() + id];
                        }
...
void AnchorGenerator::_ratio_enum(const CRect2f& anchor, const std::vector<float>& ratios, std::vector<CRect2f>& ratio_anchors) {
	...
	ratio_anchors.clear();
	float sz = w * h;
  printf("func:%s,line:%d,sz:%f, ratios.size:%d\n", __FUNCTION__, __LINE__, sz, ratios.size());
	for (int s = 0; s < ratios.size(); ++s) {
		float r = ratios[s];
		float size_ratios = sz / r;
		float ws = std::sqrt(size_ratios);
		float hs = ws * r;
    printf("func:%s, line:%d, r:%f, ws:%f, hs:%f\n", __FUNCTION__, __LINE__, r, ws, hs);
...
void AnchorGenerator::_scale_enum(const std::vector<CRect2f>& ratio_anchor, const std::vector<float>& scales, std::vector<CRect2f>& scale_anchors) {
    ...
    printf("func:%s,line:%d, scales.size:%d\n", __FUNCTION__, __LINE__, scales.size());
		for (int s = 0; s < scales.size(); ++s) {
			float ws = w * scales[s];
			float hs = h * scales[s];
      printf("func:%s, line:%d, scale:%f, ws:%f, hs:%f\n", __FUNCTION__, __LINE__, scales[s], ws, hs);
